-- السماح بربط قيد واحد بشهرين (أو أكثر) في نفس العملية
...
BEGIN
  ...
  IF p_paid AND p_amount IS NOT NULL AND p_amount > 0 AND v_month_count > 1 THEN
    v_per_month_amount := p_amount / v_month_count;
  ELSE
    v_per_month_amount := p_amount;
  END IF;
    ...
    FOREACH v_month IN ARRAY p_months LOOP
      ...
      IF EXISTS (
        ...
        WHERE trjl.journal_entry_id = p_journal_entry_id
          ...
          AND to_char(tc2.due_date, 'YYYY-MM') <> ALL (p_months)
      ) OR EXISTS (
        SELECT 1
        ...
        WHERE lc3.tenant_id = p_tenant_id
          ...
          AND to_char(tc3.due_date, 'YYYY-MM') <> ALL (p_months)
      ) THEN
      ...
      IF v_per_month_amount IS NULL OR v_per_month_amount <= 0 THEN
