...
import { useEmployeeSpendReport } from '@/lib/db/party-report-queries';
import {
...
function EmployeeSpendContent() {
  ...
  const setPeriod = useCallback(
    (next: ReportPeriod) => {
      ...
      router.replace(`/reports/employee-spend?${params.toString()}`, {
        scroll: false,
      });
    },
    [router],
  ...
  return (
    <>
      <PageHeader
        eyebrow="التقارير"
        title="إنفاق الموظفين (مرتبات)"
        description={`تجميع مصروفات مربوطة بموظفين — ${formatReportPeriodLabelAr(period)}`}
        actions={
          <div className="flex flex-wrap gap-2">
            <ExportCsvButton
              fileName={`إنفاق-موظفين-${period.year}`}
              headers={['الموظف', 'الهاتف', 'الإجمالي', 'عدد المعاملات']}
              rows={rows.map((r) => [
                r.contactName,
                r.phone ?? '',
                r.total,
            ...
            <Button variant="outline" size="sm" asChild>
              <Link href="/mall?tab=people&segment=EMPLOYEE">دليل الموظفين</Link>
            </Button>
          </div>
        }
      />
      ...
      <AccountingPageBody>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-ink-mute">إجمالي المدفوع</CardTitle>
            </CardHeader>
            <CardContent className="text-xl font-bold text-purple-800">
              {formatMoney(total, 'LYD')}
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-ink-mute">عدد الموظفين</CardTitle>
            </CardHeader>
        ...
        {isLoading ? (
          ...
          <Card className="p-8 text-center text-sm text-ink-mute">
            لا توجد مصروفات مربوطة بموظفين في هذه الفترة. سجّل راتباً من دليل الموظفين أو اربط مصروف مرتبات بموظف.
          </Card>
        ) : (
                  ...
                  {rows.map((row) => (
                    <tr key={row.contactId} className="border-b border-border/60">
                      <td className="px-4 py-3">
                        ...
                        >
                          <Briefcase className="h-3.5 w-3.5 text-ink-mute" />
                          {row.contactName}
                        </Link>
                      ...
                      <td className="px-4 py-3 text-left font-semibold tabular-nums text-purple-800">
