...
import { useVendorSpendReport } from '@/lib/db/party-report-queries';
import {
...
function VendorSpendContent() {
  ...
  const setPeriod = useCallback(
    (next: ReportPeriod) => {
      ...
      router.replace(`/reports/vendor-spend?${params.toString()}`, { scroll: false });
    },
    [router],
  );

  const { data: rows = [], isLoading, isError, error } = useVendorSpendReport(period);
  ...
  return (
    <>
      <PageHeader
        eyebrow="التقارير"
        title="إنفاق الموردين"
        description={`تجميع مصروفات مرتبطة بموردين — ${formatReportPeriodLabelAr(period)}`}
        actions={
          <div className="flex flex-wrap gap-2">
            <ExportCsvButton
              fileName={`إنفاق-موردين-${period.year}`}
              headers={['المورد', 'الهاتف', 'الإجمالي', 'عدد المعاملات']}
              rows={rows.map((r) => [
                r.contactName,
                r.phone ?? '',
                r.total,
            ...
            <Button variant="outline" size="sm" asChild>
              <Link href="/mall?tab=people&segment=VENDOR">دليل الموردين</Link>
            </Button>
          </div>
        }
      />
      ...
      <AccountingPageBody>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-ink-mute">إجمالي الإنفاق</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-ink-mute">عدد الموردين</CardTitle>
            </CardHeader>
        ...
        {isLoading ? (
          ...
          <Card className="p-8 text-center text-sm text-ink-mute">
            لا توجد مصروفات مربوطة بموردين في هذه الفترة. اربط المورد عند تسجيل المصروف أو إذن الصرف.
          </Card>
        ) : (
                  ...
                  {rows.map((row) => (
                    <tr key={row.contactId} className="border-b border-border/60">
                      <td className="px-4 py-3">
                        ...
                        >
                          <Landmark className="h-3.5 w-3.5 text-ink-mute" />
                          {row.contactName}
                        </Link>
                      ...
                      <td className="px-4 py-3 text-left font-semibold tabular-nums text-red-700">
